...
void WebSearch::doSearch(string query_word)
{
    ...
    generateQueryWordFrequenceMap(word_frequence_map, split_res);

    set<int> web_page_set;
    generateCandidateWebPageSet(web_page_set, split_res);
    if (_no_result)
        return;

    vector<double> base_vector;
    generateBaseVector(base_vector, word_frequence_map, split_res);

    map<int, vector<double>> pages_feature_ves_map;
    findCandidateWebPageID(pages_feature_ves_map, web_page_set, split_res);

    vector<pair<int, double>> cos_similar_ves;
    generateIdCosSimilarityVector(cos_similar_ves, pages_feature_ves_map, base_vector);

    vector<int> most_similar_doc_id_vec;
    generateCandidateDocIDVector(most_similar_doc_id_vec, cos_similar_ves);

    generateWebPage(most_similar_doc_id_vec);
}
